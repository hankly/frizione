...
JSDOC.SymbolSet.prototype.keys = function() {
	...
	for (var p in this._index) {
		if (this._index.hasOwnProperty(p)) {
            found.push(p);
        }
    }
	return found;
...
JSDOC.SymbolSet.prototype.resolveBorrows = function() {
	for (var p in this._index) {
        if (this._index.hasOwnProperty(p)) {
            ...
            for (var i = 0; i < borrows.length; i += 1) {
	...
	for (var p in this._index) {
        if (this._index.hasOwnProperty(p)) {
            ...
            // the memberOf value was provided in the @memberOf tag
                ...
                parts = symbol.alias.match(new RegExp("^("+symbol.memberOf+"[.#-])(.+)$"));
...
JSDOC.SymbolSet.prototype.walk = function(symbol) {
	...
	for(var i = 0; i < augments.length; i += 1) {
		...
		if (contributer) {
            ...
            if (!isUnique(symbol.inheritsFrom)) {
				...
				var ci, cl;
				for (ci = 0, cl = cmethods.length; ci < cl; ci += 1) {
					symbol.inherit(cmethods[ci]);
                }
                for (ci = 0, cl = cproperties.length; ci < cl; ci += 1) {
