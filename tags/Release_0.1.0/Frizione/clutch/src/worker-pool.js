...
(function () {
    ...
    // workerParent is the object workers should use to talk to the scope that created them.
    google.gears.workerParent = {
        ...
        sendMessage: function (message, args) {
            if (typeof args === "object") {
                message += ".json " + JSON.stringify(args);
            }
    ...
    google.gears.workerPool.onmessage = function (a, b, message) {
        var msgParts = message.text.match(/\s?(\S+)\s+(.+)/);
        if (msgParts) {
            if (msgParts[1] == "init") {
                parentId = senderId;
                return;
            ...
            var command = msgParts[1];
            var offset = command.length - ".json".length;
            return offset >= 0 && command.lastIndexOf(str) === offset;
        ...
        google.gears.workerParent.sendError("Internal worker error - Unexpected message: " + message.text);
    };
})();