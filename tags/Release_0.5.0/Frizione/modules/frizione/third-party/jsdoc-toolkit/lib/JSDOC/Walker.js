...
JSDOC.Walker.prototype.step = function() {
	if (this.token.is("JSDOC")) { // it's a doc comment
		...
		if (doc.getTag("lends").length > 0) { // it's a new namescope
			...
			matching = this.ts.getMatchingToken("LEFT_CURLY");
			if (matching) {
                matching.popNamescope = name;
            }
			else {
                LOG.warn("Mismatched } character. Can't parse code.");
            }
			...
			symbol = new JSDOC.Symbol(virtualName, [], "VIRTUAL", doc);
		...
		if (this.token.is("NAME")) {
			...
			if (this.ts.look(1).is("COLON") && this.ts.look(-1).is("LEFT_CURLY") && !(this.ts.look(-2).is("JSDOC") || this.namescope.last().comment.getTag("lends").length || this.ts.look(-2).is("ASSIGN") || this.ts.look(-2).is("COLON"))) {
				...
				matching = this.ts.getMatchingToken(null, "RIGHT_CURLY");
				if (matching) {
                    matching.popNamescope = name;
                }
				else {
			...
			// foo(...)
				...
				if (!this.ts.look(2).is("RIGHT_PAREN")) {
                    functionCall.arg1 = this.ts.look(2).data;
                }
			...
			if ((!this.ts.look(-1).is("COLON") || !this.ts.look(-1).is("ASSIGN")) && !this.ts.look(1).is("NAME")) {
				if (this.lastDoc) {
                    doc = this.lastDoc;
                }
...
JSDOC.Walker.onParamList = function(/**Array*/paramTokens) {
	...
	var params = [];
	for (var i = 0, l = paramTokens.length; i < l; i += 1) {
